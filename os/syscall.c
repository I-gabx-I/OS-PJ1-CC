...
/* User address space bounds for pointer validation */
#ifdef QEMU_MODE
    #define USR_REGION_BASE  0x00100000u
    #define USR_REGION_LIMIT 0x00220000u
#else
    #define USR_REGION_BASE  0x82100000u
    #define USR_REGION_LIMIT 0x82220000u
#endif
...
/* Copy a saved context out of the frame into a PCB. */
static void frame_to_pcb(unsigned int *frame, pcb_t *p) {
    p->sp   = frame[0];
    p->lr   = frame[1];
    p->spsr = frame[2];
...
/* Load a PCB's context into the frame for exception return. */
static void pcb_to_frame(pcb_t *p, unsigned int *frame) {
    frame[0] = p->sp;
    frame[1] = p->lr;
    frame[2] = p->spsr;
